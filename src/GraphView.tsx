...
// The octocat. Exported because RepoView's "open on GitHub" toolbar link needs
// the same mark — a 700-character path is worth having exactly one copy of.
export const GithubIcon = ({ size, className }: { size: number; className?: string }) => (
  <svg className={className} viewBox="0 0 16 16" width={size} height={size} fill="currentColor" aria-hidden="true">
      ...
      <span className="refs" onClick={e => e.stopPropagation()} onContextMenu={inert}>
        ...
        {chips.map(chip => {
          ...
          >
            ...
            {chip.remote && <GithubIcon size={11} className="ref-icon" />}
          </span>
          )
        })}
        </span>
