...
**Settings follow the same store shape as `theme.ts`, split across two files because vitest has no jsdom.** The suite runs in vitest's default node environment, so any module a test imports must not touch `document` or `localStorage`. `settings.ts` is the pure half — types, defaults, zoom clamping, corrupt-JSON/partial-object fallback, and the CSS font-stack builder — and is unit-tested directly. `settingsStore.ts` is the DOM half: it owns the `megit-settings` localStorage key and, like `theme.ts`, calls `apply()` at module import, before React mounts, so zoom and font are already correct on the first paint and the window never reflows on load. That timing is also why settings are localStorage rather than a server round-trip — a `fetch` before first paint would put a visible reflow on every load.

**Text scaling is `:root { zoom }`, not a rem sweep.** `styles.css` has dozens of hardcoded px sizes, and the graph row height is duplicated three ways — `GraphView.tsx`'s `ROW = 28`, `.row { height: 28px }`, and `contain-intrinsic-size: auto 28px`. `zoom` scales CSS pixels uniformly, so all three stay in agreement with no coordination between them; that is the load-bearing reason the settings feature never touches the graph's geometry (`lanes.ts`, or the row layout in `GraphView.tsx`). One trap it creates: `.modal`'s `max-height` is a percentage of the fixed `.modal-backdrop`, not `vh` — `vh` is resolved before `:root`'s zoom is applied and then gets scaled by it too, so a `vh` cap would overflow the viewport at high zoom.

## The pure modules

`lanes.ts`, `search.ts`, `rowNav.ts`, `branchMenu.ts`, `commitMenu.ts`, `parse.ts`, `settings.ts`, `shortcuts.ts`, and the `watch.ts` debouncer are all testable without a DOM or a real repository, and carry most of the test suite. Logic that can live in one of them should.
